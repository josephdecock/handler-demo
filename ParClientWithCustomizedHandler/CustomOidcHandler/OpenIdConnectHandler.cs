...
#pragma warning restore CS8620 // Argument cannot be used for parameter due to differences in the nullability of reference types.
    ...
    private async Task HandleChallengeAsyncInternal(AuthenticationProperties properties)
    {
        ...
        var stateDataFormattedContext = new StateDataFormattedContext(Context, Scheme, Options, message);
        await Events.StateDataFormatted(stateDataFormattedContext);
        if (redirectContext.Handled)
        {
            // TODO - Introduce new specific logger
            Logger.RedirectToIdentityProviderForSignOutHandledResponse();
            return;
        }
